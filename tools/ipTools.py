...
""" Memeriksa apakah situs web berada di bawah perlindungan CloudFlare.. """
    ...
    try:
        ...
        for i in range(len(ipv4)):
            if ipaddress.ip_address(origin) in ipaddress.ip_network(ipv4[i]):
                print(
                    f"{Fore.RED}[!] {Fore.CYAN}Situs web ini dilindungi oleh CloudFlare, serangan ini mungkin tidak berpengaruh.{Fore.RESET}"
                )
                sleep(1)
...
def __GetAddressInfo(target):
    try:
        ...
        print(f"{Fore.RED}[!] {Fore.MAGENTA}Anda harus memasukkan ip dan port{Fore.RESET}")
        sys.exit(1)
    else:
        return ip, port
