...
def GetMethodByName(method):
    if method == "SMS":
        ...
        raise SystemExit(
            f"{Fore.RED}[!] {Fore.MAGENTA}metode DDOS tidak diketahui.. {repr(method)} terpilih..{Fore.RESET}"
        )
    ...
    if hasattr(module, "flood"):
        ...
        CriticalError(
            f"Metode 'flood' tidak ditemukan di {repr(dir)}. Usa Python 3.8", "-"
        )
...
class AttackMethod:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        print(f"{Fore.MAGENTA}[!] {Fore.BLUE}Attack Berhasil!{Fore.RESET}")
    ...
    def Start(self):
        ...
        print(
            f"{Fore.MAGENTA}[?] {Fore.BLUE}Memulai serangan {target} menggunakan metode {self.name}.{Fore.RESET}\n"
            f"{Fore.MAGENTA}[?] {Fore.BLUE}Serangan akan dihentikan setelah {Fore.MAGENTA}{duration}{Fore.BLUE}.{Fore.RESET}"
        )
        ...
        try:
            ...
            self.is_running = False
            print(
                f"\n{Fore.RED}[!] {Fore.MAGENTA}Ctrl+C terdeteksi. henti {self.threads_count} Threads..{Fore.RESET}"
            )
